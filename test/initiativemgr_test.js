...
async function Test_Function()
{
  ...
  */
  // admin DB?óê?Ñú DB?ã®?úÑÎ°? USER Í¥?Î¶¨Ìï† Í≤ΩÏö∞
...
function mongo_connectDB(initiative_DB)
{
    ...
    console.log("DB ?ó∞Í≤∞ÏùÑ ?ãú?èÑ?ï©?ãà?ã§.");
    mongoose.Promise = global.Promise; // mongoose?ùò Promise Í∞ùÏ≤¥?äî global?ùò Promise Í∞ùÏ≤¥Î•? ?Ç¨?ö©?ïò?èÑÎ°? ?ï®.
    ...
    database.on('open', () => {
      ...
      /*
      // 1. User Schima ?†ï?ùò Î∞? ?†ï?òï?ôî?êú Data ????û•?ïòÍ∏?
      ...
      */
      ...
      /*
      // 2. ?†ï?òï?ôî ?ïòÍ∏? ?ñ¥?†§?ö¥ Schima Íµ¨Ï°∞ Ï¶? JSON?úºÎ°? ?êú file?ùÑ DBÎ°? Save Test....
      ...
      */
      ...
      // 3. ????û•?êú ÎπÑÏ†ï?òï?ôî JSON ObjectÎ•? DBÎ°? Î∂??Ñ∞ Find Test....
      var InitmgrSchima = mongoose.Schema({inserted_at : Date, Updated_at : Date, json: Object});    
      var InitmgrModel = mongoose.model("snapshot", InitmgrSchima);
...
/*
  load_InitiativeDB : filename(Path?†ïÎ≥? ?è¨?ï®)?ùÑ ?ùΩ?ñ¥ initiative_DB Í∞ùÏ≤¥(JSON)?úºÎ°? Loading?ïú?ã§.
*/
...
/*
  Save_JSON_file : ?†Ñ?ã¨?êú JSONÍ∞ùÏ≤¥Î•? filename(Path?†ïÎ≥? ?è¨?ï®)?úºÎ°? ????û•?ïú?ã§.
*/
